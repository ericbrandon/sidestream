...
// This file is the home for content that is true for every provider — host-app
// capabilities, markdown rendering, LaTeX conventions. All tool-name-specific
// and conditionally-gated guidance (code-exec, image-URL workflow, don't-invent-
// URLs fallback) lives in the per-provider Rust builders (providers/<name>.rs).
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        // Build system prompt: shared Part A, then user custom instructions
        // if any. Per-provider addenda (code-exec guidance, image-URL workflow
        // when web search is on, don't-invent-URLs fallback when web search is
        // off) are layered on by the Rust providers.
