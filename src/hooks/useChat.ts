...
// Cross-provider, app-level system prompt (Part A in notes/prompt_compositions.md).
// Tool-name-specific guidance lives in the per-provider request builder
// (providers/<name>.rs); image-URL "don't make one up" guidance lives in either
// the per-provider addendum (when web search is on) or in
// NO_WEB_SEARCH_IMAGE_GUIDANCE below (when web search is off).
const SYSTEM_PROMPT = `This app renders standard markdown, including images.
...
Use markdown for images, both for links to images on the web and for images generated by you.

Use LaTeX notation with $$...$$ blocks for math equations, chemical formulas, physics notation, and other expressions that require mathematical typesetting. Do not use single dollar sign $...$ delimiters. Write simple numbers, percentages, and values as plain text without LaTeX formatting.`;

// Cross-provider addendum (Part D), appended when the user has the web-search
// (globe) toggle OFF. Without web search, the model has no way to fetch a real
// image URL, so its only option for embedding one would be to guess from
// training-data memory — which 404s. This warning closes that path. When web
// search is ON, the per-provider image-URL workflow guidance covers it instead
// (ANTHROPIC_IMAGE_URL_GUIDANCE / GEMINI_IMAGE_URL_GUIDANCE), so we don't append
// this in that case.
const NO_WEB_SEARCH_IMAGE_GUIDANCE = `If trying to show the user an image on the web, don't construct image URLs from memory — they often 404.`;
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        // Build system prompt: shared Part A, then Part D (when web search is
        // off) so the model is told not to invent image URLs, then user custom
        // instructions if any. Per-provider addenda (image-URL workflow when
        // web search is on, etc.) are layered on by the Rust providers.
        let systemPrompt = SYSTEM_PROMPT;
        if (!frontierLLM.webSearchEnabled) {
          systemPrompt += `\n\n${NO_WEB_SEARCH_IMAGE_GUIDANCE}`;
        }
